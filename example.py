...
def summarize_vision_scores_one_drug(
    data_path: Annotated[
        str, typer.Option(help="path to directory containing data files")
    ],
    results_directory: Annotated[Optional[str], typer.Option(help="results directory")],
    cell_line: Annotated[
        Optional[str], typer.Option(help="cell line to analyze")
    ] = "HS-578T",
    ...
    logger.info(
        f"[summarize_vision_scores_one_drug] results directory: {results_directory}"
    )
    ...
    results_directory = (
        get_paths().results_dir / f"{provider.title()}_{model}_summaries"
    )  # type: ignore
    results_directory.mkdir(parents=True, exist_ok=True)  # type: ignore

    logger.info(
        f"[summarize_vision_scores_one_drug] single drug demonstration of vision score summary for {provider.title()} ({model})"
    )
    ...
    try:
        ...
        logger.info("[summarize_vision_scores_one_drug] analyzing vision scores for:")
        ...
        logger.info(
            f"[summarize_vision_scores_one_drug] saved model summary to: {file}"
        )
...
def summarize_vision_scores_all_drugs(
    ...
    logger.info(
        f"[summarize_vision_scores_all_drugs] results directory: {results_directory}"
    )
    ...
    logger.info(
        f"[summarize_vision_scores_all_drugs] multiple drug demonstration of vision score summary for {provider.title()} ({model})"
    )

    drugs = pd.read_csv(get_paths().drugs_file)["drug"].tolist()
    for drug_name in drugs[:5]:
        try:
            summarize_vision_scores_one_drug(
                data_path=data_path,
            ...
            logger.error(
                f"[summarize_vision_scores_all_drugs] error for {drug_name}: {e}"
            )

    analyze_embeddings(
        get_paths().results_dir / f"{provider.title()}_{model}_summaries",
        get_paths().drugs_file,
    )
...
def rank_drugs_by_summary_one_drug(
    ...
    logger.info(
        f"[rank_drugs_by_summary_one_drug] single drug demonstration of model rankings for {provider.title()} ({model})"
    )
    ...
    try:
        ...
        logger.info("[rank_drugs_by_summary_one_drug] analyzing vision scores for:")
