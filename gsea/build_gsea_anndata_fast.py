...
def arrow_select_best_plate(tab: pa.Table) -> pa.Table:
    ...
    agg = tab.group_by(["drug_conc", "cell_line", "plate"]).aggregate([("padj", "mean")])
    ...
    return tab.join(best_rows, keys=["drug_conc", "cell_line", "plate"], join_type="inner")


def build(parquet_dir: str, out: str, subset: int | None = None, threads: int = 8, debug: bool = False):
    os.environ["ARROW_NUM_THREADS"] = str(threads)
    paths = sorted(glob(f"{parquet_dir}/c_*.parquet"))
    if subset: paths = paths[:subset]
    ...
    for i, p in enumerate(paths, 1):
        ...
        pdf = tab.select(["drug_conc", "cell_line", "pathway", "NES", "padj"]).to_pandas(types_mapper=pd.ArrowDtype)
        ...
        for s, g, nes, q in zip(samples, pathways, pdf["NES"], pdf["padj"]):
            ...
            row_idx.append(r); col_idx.append(c)
            nes_vals.append(nes); padj_vals.append(q)

        del tab, pdf; gc.collect()
        dt = time.perf_counter() - t_start
        print(f"  {i:>2}/{len(paths)}  {Path(p).name:<20} {dt:>5.1f}s  rows so far {len(row_idx):,}")

    shape = (len(row_id), len(col_id))
    nes_m  = sp.coo_matrix((np.array(nes_vals,  dtype=np.float32), (row_idx, col_idx)), shape).tocsr()
    padj_m = sp.coo_matrix((np.array(padj_vals, dtype=np.float32), (row_idx, col_idx)), shape).tocsr()
    mask_m = sp.coo_matrix((np.ones(len(nes_vals), dtype=np.uint8), (row_idx, col_idx)), shape).tocsr()

    obs = (pd.Series(row_id)
             .sort_values()
    ...
    adata = ad.AnnData(X=sp.csr_matrix(shape, dtype=np.float32),
                       obs=obs, var=var,
                       layers={"gsea_nes": nes_m,
                               "gsea_padj": padj_m,
                               "gsea_mask": mask_m})
...
if __name__ == "__main__":
    ...
    ap.add_argument("--output",      default="/home/ubuntu/sigspace/results/gsea_all_cell_lines.h5ad")
